...
object SparkPageRank15 {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- 1 to iters) {
      ...
      val lines2 = spark.read.textFile(args(0)).rdd
      var ll2 = lines2.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines3 = spark.read.textFile(args(0)).rdd
      var ll3 = lines3.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines4 = spark.read.textFile(args(0)).rdd
      var ll4 = lines4.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines5 = spark.read.textFile(args(0)).rdd
      var ll5 = lines5.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines6 = spark.read.textFile(args(0)).rdd
      var ll6 = lines6.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines7 = spark.read.textFile(args(0)).rdd
      var ll7 = lines7.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines8 = spark.read.textFile(args(0)).rdd
      var ll8 = lines8.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines9 = spark.read.textFile(args(0)).rdd
      var ll9 = lines9.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines10 = spark.read.textFile(args(0)).rdd
      var ll10 = lines10.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines11 = spark.read.textFile(args(0)).rdd
      var ll11 = lines11.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines12 = spark.read.textFile(args(0)).rdd
      var ll12 = lines12.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines13 = spark.read.textFile(args(0)).rdd
      var ll13 = lines13.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines14 = spark.read.textFile(args(0)).rdd
      var ll14 = lines14.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines15 = spark.read.textFile(args(0)).rdd
      var ll15 = lines15.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()
      ...
      val ranks10 = contribs10.reduceByKey(_ + _)

      val contribs11 = ll12.join(ranks10).values.flatMap{ case (urls, rank) =>
        val size = urls.size
        urls.map(url => (url, rank / size))
      }

      val ranks11 = contribs11.reduceByKey(_ + _)

      val contribs12 = ll13.join(ranks11).values.flatMap{ case (urls, rank) =>
        val size = urls.size
        urls.map(url => (url, rank / size))
      }

      val ranks12 = contribs12.reduceByKey(_ + _)

      val contribs13 = ll14.join(ranks12).values.flatMap{ case (urls, rank) =>
        val size = urls.size
        urls.map(url => (url, rank / size))
      }

      val ranks13 = contribs13.reduceByKey(_ + _)

      val contribs14 = ll15.join(ranks13).values.flatMap{ case (urls, rank) =>
        val size = urls.size
        urls.map(url => (url, rank / size))
      }

      val ranks14 = contribs14.reduceByKey(_ + _).collect()
    }
