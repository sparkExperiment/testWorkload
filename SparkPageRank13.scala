...
object SparkPageRank13 {
  ...
  def main(args: Array[String]) {
    ...
    for (i <- 1 to iters) {
      ...
      val lines12 = spark.read.textFile(args(12)).rdd
      var ll12 = lines12.map{ s =>
        val parts = s.split("\\s+")
        (parts(0), parts(1))
      }.groupByKey()

      val lines13 = spark.read.textFile(args(13)).rdd
      ...
      val ranks12 = contribs12.reduceByKey(_ + _).collect()
    }
